...
class ShoppingCartPage:
    def __init__(self, driver: WebDriver):
        ...
        self.shopping_cart_link = self.driver.find_element(
            By.CSS_SELECTOR, '[data-test="shopping-cart-link"]'
        )
        self.burger_menu_button = self.driver.find_element(
            By.ID, "react-burger-menu-btn"
        )
        self.log_out = self.driver.find_element(
            By.CSS_SELECTOR, '[data-test="logout-sidebar-link"]'
        )
        self.shopping_cart_button = self.driver.find_element(
            By.ID, "shopping_cart_container"
        )
        # -----------------------------
        # List of elements
        # -----------------------------
        self.products_cart = self.driver.find_elements(
    ...
    def add_first_product_to_cart(self):
        ...
        assert (
            len(products_to_add_cart) > 0
        ), "No hay productos disponibles para agregar al carrito."
